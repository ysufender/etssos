...
void Kernel_IO_PrintFormat_Privileged(char const* const fmt, ...) {
    va_list args;
...
void Kernel_IO_PutString_Privileged(char const* const str) {
    for (uint32_t i = 0; str[i] != '\0'; i++) {
        Drivers_UART_PutChar(str[i]);
    }
}

void Kernel_IO_PutStringLine_Privileged(char const* const str) {
    for (uint32_t i = 0; str[i] != '\0'; i++) {
        Drivers_UART_PutChar(str[i]);
    }
    Drivers_UART_PutChar('\n');
